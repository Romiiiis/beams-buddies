...
const TYPE = {
  ...
  valueLg: { fontSize: '28px', fontWeight: 900, letterSpacing: '-0.05em' as const, lineHeight: 1, color: TEXT },
}
...
function IconArrowLeft({ size = 15 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M19 12H5M11 18l-6-6 6-6" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" /></svg>
}
...
function IconEdit({ size = 15 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 20h9" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" /><path d="m16.5 3.5 4 4L7 21l-4 1 1-4L16.5 3.5Z" stroke="currentColor" strokeWidth="1.9" strokeLinejoin="round" /></svg>
}
function IconTrash({ size = 15 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 6h18" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" /><path d="M8 6V4h8v2M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6" stroke="currentColor" strokeWidth="1.9" strokeLinejoin="round" /><path d="M10 11v6M14 11v6" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" /></svg>
}
function IconMail({ size = 16 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><rect x="3" y="5" width="18" height="14" rx="2.5" stroke="currentColor" strokeWidth="1.9" /><path d="m4 7 8 6 8-6" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" /></svg>
}
...
function IconMapPin({ size = 16 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M12 21s-6-5.33-6-11a6 6 0 1 1 12 0c0 5.67-6 11-6 11Z" stroke="currentColor" strokeWidth="1.9" strokeLinejoin="round" /><circle cx="12" cy="10" r="2.5" stroke="currentColor" strokeWidth="1.9" /></svg>
}
function IconClock({ size = 16 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><circle cx="12" cy="12" r="9" stroke="currentColor" strokeWidth="1.9" /><path d="M12 7v5l3 2" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round" /></svg>
}
function IconExternalLink({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M7 17L17 7M17 7H7M17 7v10" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
}

function StatImageIcon({ src, alt }: { src: string; alt: string }) {
  return <img src={src} alt={alt} style={{ width: '30px', height: '30px', objectFit: 'contain', display: 'block', flexShrink: 0 }} />
}
...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const totalServiceRecords = useMemo(() => jobs.reduce((s, j) => s + (j.service_records?.length || 0), 0), [jobs])
  ...
  const stats = useMemo(() => {
    ...
    return { jobs: jobs.length, serviceRecords: totalServiceRecords, reviewClicks: reviewClicks.length, dueSoon, overdue }
  ...
  const textareaStyle: React.CSSProperties = { ...inputStyle, height: '88px', padding: '10px 12px', resize: 'none' }
  ...
  const sideCard: React.CSSProperties = { ...card, padding: '16px', borderRadius: '16px' }
  ...
  const topCards = [
    ...
    },
    {
      ...
      sub: uniquePlatforms.length > 0 ? `${uniquePlatforms.length} platform${uniquePlatforms.length === 1 ? '' : 's'}` : 'No activity yet',
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', minHeight: '100vh', background: BG, fontFamily: FONT }}>
        ...
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', color: TEXT3, fontSize: '14px', fontWeight: 600 }}>Loading customer...</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', fontFamily: FONT, background: BG, minHeight: '100vh' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, background: BG }}>
        ...
        >
          {/* ── HEADER: edge-to-edge + flush to top on mobile ── */}
          <div
            style={{
              ...card,
              ...
              ...(isMobile ? { borderRadius: 0, marginLeft: '-14px', marginRight: '-14px', marginTop: '-14px' } : {}),
            }}
          >
            <button
              onClick={() => router.push('/dashboard/customers')}
              style={{ background: 'transparent', border: 'none', padding: 0, margin: 0, color: 'rgba(255,255,255,0.68)', display: 'inline-flex', alignItems: 'center', gap: '7px', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: FONT, marginBottom: '10px' }}
            >
            ...
            <div style={{ marginTop: '14px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <button
                ...
                style={{ height: '36px', padding: '0 14px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '7px', background: TEAL, color: WHITE, border: 'none', borderRadius: '10px' }}
              >
              ...
              <button
                ...
                style={{ height: '36px', padding: '0 14px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '7px', background: 'rgba(255,255,255,0.06)', color: WHITE, border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px' }}
              >
                <IconEdit size={14} /> Edit customer
              </button>
              ...
              <button
                ...
                style={{ height: '36px', padding: '0 14px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '7px', background: 'rgba(255,255,255,0.06)', color: WHITE, border: '1px solid rgba(255,255,255,0.10)', borderRadius: '10px' }}
              >
                <IconTrash size={14} /> Delete customer
              </button>

              {saved && (
                <div style={{ height: '36px', padding: '0 14px', borderRadius: '10px', background: 'rgba(255,255,255,0.12)', color: WHITE, display: 'inline-flex', alignItems: 'center', fontSize: '12px', fontWeight: 700 }}>
                  Saved
                </div>
              )}
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, 1fr)', gap: '10px' }}>
            {topCards.map(item => (
              <div key={item.label} style={statCard}>
                ...
                <div>
                  <div style={{ fontSize: isMobile ? '22px' : '26px', fontWeight: 900, letterSpacing: '-0.05em', lineHeight: 1, color: item.accent }}>{item.value}</div>
                  <div style={{ fontSize: isMobile ? '10px' : '11px', fontWeight: 500, color: TEXT3, marginTop: '3px', lineHeight: 1.4 }}>{item.sub}</div>
                </div>
              </div>
            ))}
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '360px 1fr', gap: '14px', alignItems: 'start' }}>
            <div style={{ display: 'grid', gap: '14px' }}>
              <div style={card}>
                <div style={{ padding: '14px 16px 12px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px' }}>
                  <div>
                    <div style={sectionHeaderTitle}>Customer profile</div>
                    <div style={{ ...TYPE.bodySm }}>Contact details, address, and internal notes.</div>
                  </div>
                  {!editingCustomer && (
                    <button onClick={() => setEditingCustomer(true)} style={cardArrowBtn}><IconExternalLink size={14} /></button>
                  )}
                </div>
                ...
                {!editingCustomer ? (
                  <div style={{ display: 'grid' }}>
                    {[
                      ...
                      <div key={row.label} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', padding: '13px 18px', borderBottom: index === 4 && !customer.notes ? 'none' : `1px solid ${BORDER}` }}>
                  ...
                  <div style={{ padding: '18px', display: 'grid', gap: '10px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                      <button onClick={() => setEditingCustomer(false)} style={{ flex: 1, height: '38px', borderRadius: '10px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>Cancel</button>
                      <button onClick={saveCustomer} disabled={saving} style={{ flex: 1, height: '38px', borderRadius: '10px', border: 'none', background: TEAL, color: WHITE, fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>{saving ? 'Saving...' : 'Save'}</button>
                    </div>
                  </div>
                )}
              </div>
              ...
              {(reviewClicks.length > 0 || jobs.length > 0) && (
                <div style={sideCard}>
                  ...
                  <div style={{ fontSize: '22px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em', marginBottom: '14px' }}>
                    {stats.overdue > 0 ? <><span style={{ color: TEXT }}>{stats.overdue}</span> Overdue unit{stats.overdue !== 1 ? 's' : ''}</> : <span style={{ color: TEAL_DARK }}>All Clear</span>}
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {[
                      { label: 'Overdue', val: stats.overdue, bg: '#FFF9F9', border: '#F3D4D4' },
                      { label: 'Due soon', val: stats.dueSoon, bg: '#FFFDF7', border: '#F4E5B8' },
                      { label: 'Review clicks', val: stats.reviewClicks, bg: '#FCFCFD', border: BORDER },
                    ].map(row => (
                      <div key={row.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', padding: '10px 12px', borderRadius: '10px', background: row.bg, border: `1px solid ${row.border}` }}>
                ...
                <div style={card}>
                  ...
                  <div style={{ display: 'grid' }}>
                    {uniquePlatforms.map((platform, index) => {
                      ...
                      return (
                        <div key={platform as string} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', padding: '12px 18px', borderBottom: index === uniquePlatforms.length - 1 ? 'none' : `1px solid ${BORDER}` }}>
                          <div>
            ...
            <div style={card}>
              <div style={{ padding: '14px 16px 12px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: isMobile ? 'stretch' : 'center', justifyContent: 'space-between', flexDirection: isMobile ? 'column' : 'row', gap: '10px' }}>
                ...
                <div style={{ height: '34px', borderRadius: '10px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, fontSize: '12px', fontWeight: 700, padding: '0 12px', display: 'inline-flex', alignItems: 'center', gap: '8px', fontFamily: FONT }}>
                  {jobs.length} unit{jobs.length !== 1 ? 's' : ''}
                </div>
              </div>

              {jobs.length === 0 ? (
                <div style={{ padding: '32px 18px', textAlign: 'center', color: TEXT3, fontSize: '13px' }}>No jobs yet for this customer.</div>
              ) : (
                <div style={{ display: 'grid' }}>
                  {jobs.map((job, idx) => {
                    ...
                    return (
                      <div key={job.id} style={{ borderBottom: idx === jobs.length - 1 ? 'none' : `1px solid ${BORDER}` }}>
                        <div style={{ padding: '14px 18px', display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: '10px', flexDirection: isMobile ? 'column' : 'row', borderBottom: !isEditing ? `1px solid ${BORDER}` : 'none', background: WHITE }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
                            <div style={{ width: 38, height: 38, borderRadius: '10px', background: avColors[idx % avColors.length].bg, color: avColors[idx % avColors.length].color, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: 800, flexShrink: 0 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                            ...
                            {!isEditing && (
                              <button onClick={() => setEditingJobId(job.id)} style={{ height: '34px', borderRadius: '10px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, fontSize: '12px', fontWeight: 700, padding: '0 12px', cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '7px' }}>
                        ...
                        {!isEditing ? (
                          <>
                            <div style={{ padding: '14px 18px', display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(3, minmax(0,1fr))', gap: '10px' }}>
                              {[
                                { label: 'Serial number', value: job.serial_number || '—', mono: true, icon: <IconArrow size={12} /> },
                                { label: 'Installed', value: job.install_date ? new Date(job.install_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) : '—', icon: <IconClock size={14} /> },
                                { label: 'Next service', value: job.next_service_date ? new Date(job.next_service_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) : '—', danger: job.next_service_date && getDays(job.next_service_date) < 0, icon: <IconClock size={14} /> },
                                { label: 'Warranty expiry', value: job.warranty_expiry ? new Date(job.warranty_expiry).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) : '—', icon: <IconClock size={14} /> },
                                { label: 'Location', value: job.install_location || '—', icon: <IconMapPin size={14} /> },
                                { label: 'Service interval', value: `Every ${job.service_interval_months} months`, icon: <IconClock size={14} /> },
                              ].map(row => (
                                <div key={row.label} style={{ borderRadius: '12px', border: `1px solid ${BORDER}`, background: '#FCFCFD', padding: '12px' }}>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '7px', color: TEXT3 }}>
                                    {row.icon}
                                    <div style={TYPE.label}>{row.label}</div>
                                  </div>
                                  <div style={{ ...TYPE.body, fontWeight: 700, color: row.danger ? RED : TEXT, fontFamily: row.mono ? 'ui-monospace, SFMono-Regular, Menlo, monospace' : FONT, wordBreak: 'break-word' }}>
                                    {row.value}
                                  </div>
                                </div>
                              ))}
                            ...
                            {job.notes && (
                              <div style={{ padding: '0 18px 14px' }}>
                                <div style={{ borderRadius: '12px', border: `1px solid ${BORDER}`, background: '#FCFCFD', padding: '12px' }}>
                                  <div style={{ ...TYPE.label, marginBottom: '6px' }}>Notes</div>
                          ...
                          <div style={{ padding: '18px', display: 'grid', gap: '10px', borderTop: `1px solid ${BORDER}` }}>
                            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '10px' }}>
                              <div><label style={labelStyle}>Brand</label><input style={inputStyle} value={f.brand || ''} onChange={e => setJobField(job.id, 'brand', e.target.value)} /></div>
                              <div><label style={labelStyle}>Model</label><input style={inputStyle} value={f.model || ''} onChange={e => setJobField(job.id, 'model', e.target.value)} /></div>
                              <div><label style={labelStyle}>Capacity (kW)</label><input style={inputStyle} value={f.capacity_kw || ''} onChange={e => setJobField(job.id, 'capacity_kw', e.target.value)} /></div>
                              <div>
                              ...
                              <div><label style={labelStyle}>Serial number</label><input style={inputStyle} value={f.serial_number || ''} onChange={e => setJobField(job.id, 'serial_number', e.target.value)} /></div>
                              <div><label style={labelStyle}>Location</label><input style={inputStyle} value={f.install_location || ''} onChange={e => setJobField(job.id, 'install_location', e.target.value)} /></div>
                              <div><label style={labelStyle}>Install date</label><input type="date" style={inputStyle} value={f.install_date?.slice(0, 10) || ''} onChange={e => setJobField(job.id, 'install_date', e.target.value)} /></div>
                              <div><label style={labelStyle}>Warranty expiry</label><input type="date" style={inputStyle} value={f.warranty_expiry?.slice(0, 10) || ''} onChange={e => setJobField(job.id, 'warranty_expiry', e.target.value)} /></div>
                              <div>
                            ...
                            <div><label style={labelStyle}>Notes</label><textarea style={textareaStyle} value={f.notes || ''} onChange={e => setJobField(job.id, 'notes', e.target.value)} /></div>
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <button onClick={() => setEditingJobId(null)} style={{ flex: 1, height: '38px', borderRadius: '10px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>Cancel</button>
                              <button onClick={() => saveJob(job.id)} disabled={saving} style={{ flex: 1, height: '38px', borderRadius: '10px', border: 'none', background: TEAL, color: WHITE, fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>{saving ? 'Saving...' : 'Save changes'}</button>
                            </div>
                          </div>
                        )}
                          ...
                          <div style={{ borderTop: `1px solid ${BORDER}`, background: '#FAFBFC' }}>
                            ...
                            <div style={{ display: 'grid' }}>
                              {job.service_records.map((sr: any, srIndex: number) => (
                                <div key={sr.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px', padding: '12px 18px', borderTop: srIndex === 0 ? 'none' : `1px solid ${BORDER}`, background: WHITE }}>
                                  <div style={{ minWidth: 0 }}>
                                    <div style={TYPE.titleSm}>{sr.service_type?.replace('_', ' ')}</div>
                                    {sr.notes && <div style={{ ...TYPE.bodySm, marginTop: '4px' }}>{sr.notes}</div>}
                                  </div>
                                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                    <div style={TYPE.body}>{new Date(sr.service_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
      ...
      {showDeleteConfirm && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,10,10,0.45)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: WHITE, borderRadius: '16px', width: '100%', maxWidth: '420px', border: `1px solid ${BORDER}`, overflow: 'hidden', fontFamily: FONT }}>
            ...
            <div style={{ padding: '24px 24px 20px' }}>
              ...
              {deleteError && (
                <div style={{ marginTop: '14px', padding: '12px 14px', background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '8px', fontSize: '13px', color: '#B91C1C', fontWeight: 500 }}>{deleteError}</div>
              )}
            </div>
            <div style={{ padding: '0 24px 24px', display: 'flex', gap: '10px' }}>
              <button onClick={() => { setShowDeleteConfirm(false); setDeleteError('') }} style={{ flex: 1, height: '42px', borderRadius: '9px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, fontSize: '14px', fontWeight: 600, cursor: 'pointer', fontFamily: FONT }}>Cancel</button>
              <button onClick={deleteCustomer} disabled={deleting} style={{ flex: 1, height: '42px', borderRadius: '9px', border: 'none', background: '#EF4444', color: WHITE, fontSize: '14px', fontWeight: 700, cursor: deleting ? 'not-allowed' : 'pointer', fontFamily: FONT, opacity: deleting ? 0.7 : 1 }}>
