...
export default function CustomersPage() {
  ...
  function statusPill(jobs: any[]) {
    if (!jobs?.length) {
      return { label: 'No units', bg: '#F1F5F9', color: TEXT3, border: BORDER }
    }
    ...
    if (!next) {
      return { label: 'No date', bg: '#F1F5F9', color: TEXT3, border: BORDER }
    }

    const days = getDays(next)

    if (days < 0) {
      return { label: 'Overdue', bg: '#FEE2E2', color: '#991B1B', border: '#FECACA' }
    }

    if (days <= 30) {
      return { label: 'Due soon', bg: '#FEF3C7', color: '#92400E', border: '#FDE68A' }
    }
        ...
        >
          ...
          >
            {statChips.map((chip, i) => (
              ...
              >
                <div style={{ fontSize: isMobile ? '16px' : '20px', fontWeight: 800, color: TEXT2, letterSpacing: '-0.03em', lineHeight: 1, fontVariantNumeric: 'tabular-nums' }}>
                  {chip.value}
                </div>
                <div style={{ fontSize: '11px', fontWeight: 700, color: TEXT, marginTop: '3px', lineHeight: 1.2 }}>
                  {chip.label}
                </div>
                {chip.sub && (
          ...
          <div style={card}>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((c) => {
                ...
                const unitLine =
                  latestUnit?.brand || latestUnit?.model
                    ? `${latestUnit?.brand || ''} ${latestUnit?.model || ''}`.trim()
                    : 'No unit details'
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      margin: isMobile ? '6px 12px' : '10px 12px',
                      padding: '14px 16px',
                      border: `1px solid ${BORDER}`,
                      borderRadius: '12px',
                      background: WHITE,
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                              ...
                              {unitLine !== 'No unit details' && (
                                <span style={{ fontSize: '10px', color: TEXT3 }}>· {unitLine}</span>
                              )}
                            </div>
                            ...
                            <span
                              style={{
                                fontSize: '10px',
                                fontWeight: 700,
                                color: serviceDate === 'Not set' ? TEXT3 : TEAL_DARK,
                              }}
                            >
