  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', transform: 'rotate(-90deg)' }}>
        ...
        {arcs.map(arc => (
          <circle key={arc.label} cx={cx} cy={cy} r={r} fill="none" stroke={arc.color} strokeWidth={hovered === arc.label ? thickness + 5 : thickness} strokeDasharray={`${arc.sw} ${circ}`} strokeDashoffset={-arc.s} strokeLinecap="butt" style={{ transition: 'all 0.18s', opacity: hovered && hovered !== arc.label ? 0.3 : 1, cursor: 'pointer' }} onMouseEnter={() => setHovered(arc.label)} onMouseLeave={() => setHovered(null)} />
        ))}
      </svg>
  ...
  return (
    <div style={{ display: 'flex', gap: '3px', alignItems: 'flex-end', height: H + 18 }}>
      {data.map((item, i) => {
        ...
        return (
          <div key={item.label} onMouseEnter={() => setHovered(item.label)} onMouseLeave={() => setHovered(null)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, cursor: 'default', position: 'relative' }}>
            {isHov && item.total > 0 && (
              <div style={{ position: 'absolute', bottom: H + 4, left: '50%', transform: 'translateX(-50%)', fontSize: '10px', fontWeight: 800, color: WHITE, background: TEXT, padding: '2px 5px', borderRadius: '5px', whiteSpace: 'nowrap', zIndex: 5 }}>
                {item.total}
              </div>
            )}
            <div style={{ width: '100%', height: barH, borderRadius: '4px 4px 2px 2px', background: isCurrent ? TEAL : isHov ? '#CBD5E1' : '#E2E8F0', opacity: isCurrent ? 1 : 0.9, marginTop: 'auto', transition: 'background 0.15s ease' }} />
            <span style={{ fontSize: '8px', fontWeight: 700, color: isCurrent ? TEAL_DARK : TEXT3 }}>{item.label}</span>
          </div>
        )
      })}
    </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const overdue = jobs.filter(j => { if (!j.next_service_date) return false; return startOfDay(new Date(j.next_service_date)) < today })
      ...
      const jobsToday = jobs.filter(j => { if (!j.next_service_date) return false; const d = startOfDay(new Date(j.next_service_date)); return d.getTime() === today.getTime() }).length
      const upcomingJobs = jobs.filter(j => { if (!j.next_service_date) return false; return startOfDay(new Date(j.next_service_date)) >= today }).slice(0, 5)
  ...
  const todayAppointmentsCount = useMemo(() => allJobs.filter(j => j.next_service_date && getDays(j.next_service_date) === 0).length, [allJobs])
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4, 1fr)', gap: '12px' }}>
            <div style={cardP}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                <div style={{ ...TYPE.label }}>Jobs This Month</div>
              ...
              <div style={{ fontSize: '22px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em', marginBottom: '14px' }}>
                ...
                <span style={{ fontSize: '14px', fontWeight: 600, color: TEXT3 }}> jobs created</span>
              </div>
              ...
              <button onClick={() => router.push('/dashboard/schedule')} style={{ width: '100%', height: '36px', background: TEAL, color: WHITE, border: 'none', borderRadius: '10px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT }}>
                View schedule
              </button>
            </div>
            ...
            <div style={cardP}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                <div style={{ ...TYPE.label }}>Revenue This Month</div>
                <button onClick={() => router.push('/dashboard/revenue')} style={cardArrowBtn()}><IconExternalLink size={14} /></button>
              </div>
              <div style={{ marginBottom: '4px' }}>
                <span style={{ fontSize: '26px', fontWeight: 900, color: TEXT, letterSpacing: '-0.05em' }}>${invoiceStats.collected > 0 ? (invoiceStats.collected / 1000).toFixed(1) + 'k' : '0'}</span>
                <span style={{ fontSize: '12px', fontWeight: 600, color: TEXT3, marginLeft: 6 }}>collected</span>
              </div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', marginBottom: '10px', padding: '3px 7px', borderRadius: '6px', background: '#E6F7F6' }}>
                ...
                <span style={{ fontSize: '11px', fontWeight: 800, color: TEAL_DARK }}>${invoiceStats.outstanding > 0 ? (invoiceStats.outstanding / 1000).toFixed(1) + 'k' : '0'} outstanding</span>
              </div>
            ...
            <div style={cardP}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px' }}>
                <div style={{ ...TYPE.label }}>Total Customers</div>
                <button onClick={() => router.push('/dashboard/customers')} style={cardArrowBtn()}><IconExternalLink size={14} /></button>
              </div>
              <div style={{ marginBottom: '4px' }}>
                <span style={{ fontSize: '26px', fontWeight: 900, color: TEXT, letterSpacing: '-0.05em' }}>{stats.customers > 0 ? (stats.customers >= 1000 ? (stats.customers / 1000).toFixed(1) + 'k' : stats.customers) : '0'}</span>
                <span style={{ fontSize: '12px', fontWeight: 600, color: TEXT3, marginLeft: 6 }}>in CRM</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '14px' }}>
                {[
                  { dot: TEAL, label: 'Jobs today', val: stats.jobsToday },
                  { dot: TEAL_DARK, label: 'Scheduled jobs', val: scheduledCount },
                  { dot: '#94A3B8', label: 'Units tracked', val: stats.units },
                ].map(row => (
                  <div key={row.label} style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: row.dot, flexShrink: 0 }} />
                    <span style={{ fontSize: '11px', fontWeight: 600, color: TEXT2, flex: 1 }}>{row.label}</span>
                    <span style={{ fontSize: '11px', fontWeight: 700, color: TEXT }}>{row.val}</span>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: '14px', paddingTop: '12px', borderTop: `1px solid ${BORDER}`, fontSize: '11px', fontWeight: 600, color: TEXT3 }}>Live dashboard totals</div>
            </div>
            ...
            <div style={cardP}>
              ...
              <div style={{ fontSize: '22px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em', marginBottom: '16px' }}>
                {stats.overdue > 0 ? <><span style={{ color: '#B91C1C' }}>{stats.overdue}</span> overdue service{stats.overdue !== 1 ? 's' : ''}</> : <span style={{ color: TEAL }}>All Clear</span>}
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px' }}>
                <div onClick={() => router.push('/dashboard/jobs')} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', background: '#FEF2F2', border: '1px solid #FECACA', cursor: 'pointer' }}>
                ...
                <div onClick={() => router.push('/dashboard/invoices')} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', background: '#FEF2F2', border: '1px solid #FECACA', cursor: 'pointer' }}>
                  <span style={{ color: '#B91C1C', flexShrink: 0 }}><IconAlert size={15} /></span>
                  <div style={{ flex: 1, fontSize: '12px', fontWeight: 700, color: '#7F1D1D' }}>
                    {invoiceStats.outstanding > 0 ? <><span style={{ fontWeight: 900 }}>${invoiceStats.outstanding.toLocaleString('en-AU', { minimumFractionDigits: 0 })}</span> overdue invoices</> : 'No overdue invoices'}
                  </div>
                  <IconChevronRight size={13} />
                </div>
              </div>
              <button onClick={() => router.push('/dashboard/jobs')} style={{ width: '100%', height: '34px', background: '#F8FAFC', border: `1px solid ${BORDER}`, borderRadius: '10px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, color: TEXT2 }}>View all</button>
            </div>
          </div>
            ...
            <div style={card}>
              <div style={{ padding: '16px 18px 12px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                  {[{ label: 'Today', val: todayAppointmentsCount }, { label: 'Due', val: stats.overdue }, { label: 'Scheduled', val: scheduledCount }, { label: 'Soon', val: dueSoonCount }].map(item => (
              ...
              {upcoming.length === 0 ? (
                ...
                return (
                  <div key={job.id} onClick={() => router.push(`/dashboard/customers/${job.customer_id}`)} style={{ display: 'grid', gridTemplateColumns: isMobile ? '60px 1fr auto' : '72px 1fr auto auto', gap: '10px', alignItems: 'center', padding: '13px 18px', borderBottom: `1px solid ${BORDER}`, cursor: 'pointer', transition: 'background 0.12s' }} onMouseEnter={e => (e.currentTarget.style.background = '#F8FAFC')} onMouseLeave={e => (e.currentTarget.style.background = WHITE)}>
                    ...
                    {isFirst ? (
                      <button onClick={e => { e.stopPropagation(); router.push(`/dashboard/customers/${job.customer_id}`) }} style={{ height: '30px', padding: '0 10px', background: '#F8FAFC', border: `1px solid ${BORDER}`, borderRadius: '8px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, color: TEXT2, display: 'inline-flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px', flexWrap: 'wrap', gap: '8px' }}>
                  <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                    {([['next7', 'Next 7 Days'], ['due', 'Jobs Due'], ['attime', 'At time']] as const).map(([key, lbl]) => (
                      <button key={key} onClick={() => setActiveTab(key)} style={{ height: '28px', padding: '0 9px', borderRadius: '7px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, border: `1px solid ${activeTab === key ? TEAL : BORDER}`, background: activeTab === key ? '#E6F7F6' : WHITE, color: activeTab === key ? TEAL_DARK : TEXT3 }}>
                        {lbl}
                      </button>
                    ))}
                  </div>
                  <button onClick={() => router.push('/dashboard/jobs')} style={{ height: '28px', padding: '0 9px', background: '#F8FAFC', border: `1px solid ${BORDER}`, borderRadius: '7px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, color: TEXT2, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
            ...
            <div style={cardP}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {revenueBreakdown.map(rb => (
                  <div key={rb.label} onClick={() => router.push('/dashboard/revenue')} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 0', borderBottom: `1px solid ${BORDER}`, cursor: 'pointer' }}>
              ...
              <div style={{ marginTop: '12px', padding: '12px', borderRadius: '11px', background: '#E6F7F6', border: '1px solid #C4E8E5' }}>
                ...
                <div style={{ fontSize: '22px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em' }}>${invoiceStats.collected > 0 ? invoiceStats.collected.toLocaleString('en-AU', { minimumFractionDigits: 0 }) : '0'}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 340px', gap: '14px', alignItems: 'start' }}>
            <div style={card}>
              ...
              {recent.length === 0 ? (
                ...
                return (
                  <div key={job.id} onClick={() => router.push(`/dashboard/customers/${job.customer_id}`)} style={{ display: 'grid', gridTemplateColumns: isMobile ? 'auto 1fr auto' : 'auto 1fr 120px 110px auto', gap: '12px', alignItems: 'center', padding: '12px 18px', borderBottom: `1px solid ${BORDER}`, cursor: 'pointer', transition: 'background 0.12s' }} onMouseEnter={e => (e.currentTarget.style.background = '#F8FAFC')} onMouseLeave={e => (e.currentTarget.style.background = WHITE)}>
                    ...
                    {!isMobile && (
                      <>
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: '12px', fontWeight: 600, color: TEXT2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{job.brand || 'Unit'}{job.capacity_kw ? ` ${job.capacity_kw}kW` : ''}</div>
                        </div>
                        <div>
                          ...
                          <div style={{ fontSize: '12px', fontWeight: 600, color: TEXT2 }}>{job.next_service_date ? new Date(job.next_service_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Not set'}</div>
                        </div>
                      </>
                    )}
            ...
            <div style={card}>
              <div style={{ padding: '14px 18px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <button onClick={() => router.push('/dashboard/invoices')} style={{ height: '28px', padding: '0 9px', background: '#F8FAFC', border: `1px solid ${BORDER}`, borderRadius: '7px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, color: TEXT2, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
                  View all <IconArrow size={11} />
                </button>
              </div>
              <div style={{ padding: '12px 18px', borderBottom: `1px solid ${BORDER}` }}>
                <div style={{ fontSize: '22px', fontWeight: 900, color: invoiceStats.outstanding > 0 ? '#991B1B' : TEXT, letterSpacing: '-0.04em' }}>${invoiceStats.outstanding > 0 ? invoiceStats.outstanding.toLocaleString('en-AU', { minimumFractionDigits: 0 }) : '0'}</div>
              ...
              {invoiceStats.allInvoices.length === 0 ? (
                ...
                return (
                  <div key={inv.id || i} onClick={() => router.push('/dashboard/invoices')} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 18px', borderBottom: `1px solid ${BORDER}`, cursor: 'pointer', transition: 'background 0.12s' }} onMouseEnter={e => (e.currentTarget.style.background = '#F8FAFC')} onMouseLeave={e => (e.currentTarget.style.background = WHITE)}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: '10px', color: TEXT3, marginTop: '1px' }}>{inv.created_at ? new Date(inv.created_at).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' }) : ''} · {inv.job_type || 'Invoice'}</div>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      ...
                      <span style={{ fontSize: '10px', fontWeight: 700, padding: '2px 6px', borderRadius: '5px', background: isOverdue ? '#FEE2E2' : '#FEF3C7', color: isOverdue ? '#991B1B' : '#92400E' }}>{isOverdue ? 'Overdue' : 'Sent'}</span>
                    </div>
                  </div>
                )
              })}
