      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflowY: 'auto' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: isMobile ? '28px' : '34px', fontWeight: '800', color: WHITE, letterSpacing: '-0.8px', lineHeight: 1 }}>Customers</div>
          </div>
        ...
        {/* BODY */}
        <div style={{ padding: `28px ${pad}`, paddingBottom: isMobile ? '90px' : '40px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap' }}>
            ...
            />
            <div style={{ fontSize: '13px', color: TEXT3, fontWeight: '600' }}>{customers.length} total</div>
          </div>
          ...
          {loading ? (
            ...
            <div style={{ padding: '64px', textAlign: 'center', color: TEXT3, fontSize: '14px' }}>
              No customers yet. <span style={{ color: TEAL, cursor: 'pointer', fontWeight: '600' }} onClick={() => router.push('/dashboard/jobs')}>Add your first job →</span>
            </div>
          ) : isMobile ? (
              ...
              {customers.map((c, i) => {
                ...
                return (
                  <div key={c.id} onClick={() => router.push(`/dashboard/customers/${c.id}`)}
                    style={{ ...card, padding: '14px 16px', cursor: 'pointer', borderRadius: '12px' }}>
            ...
            <div style={card}>
              <div style={{ padding: '14px 22px', borderBottom: `1px solid ${BORDER}`, background: '#FAFAFA' }}>
                <div style={{ fontSize: '11px', fontWeight: '700', color: TEAL, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '2px' }}>Directory</div>
                <div style={{ fontSize: '16px', fontWeight: '700', color: TEXT }}>All customers</div>
              </div>
                  ...
                  {customers.map((c, i) => {
                    ...
                    return (
                      <tr key={c.id} style={{ borderBottom: `1px solid ${BORDER}`, cursor: 'pointer' }}
                        ...
                        <td style={{ padding: '14px 22px', fontSize: '13px', color: TEXT2, fontWeight: '500' }}>{c.phone || '—'}</td>
                        <td style={{ padding: '14px 22px', fontSize: '13px', color: TEXT2, textAlign: 'center' }}>{c.jobs?.length || 0}</td>
                          ...
                          {totalPlatforms > 0 ? (
                            <div style={{ display: 'flex', alignItems: 'center', gap: '7px' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: hasClicks ? '#FEF3C7' : '#F5F5F5', padding: '3px 9px', borderRadius: '20px', border: `1px solid ${hasClicks ? '#FDE68A' : BORDER}` }}>
                                ...
                                <span style={{ fontSize: '12px', fontWeight: '700', color: hasClicks ? '#92400E' : '#9CA3AF' }}>{clicks}/{totalPlatforms}</span>
                              </div>
                        ...
                        <td style={{ padding: '14px 22px' }}>
                          <span style={{ background: s.bg, color: s.color, padding: '4px 11px', borderRadius: '20px', fontSize: '12px', fontWeight: '700' }}>{s.label}</span>
                        </td>
                        <td style={{ padding: '14px 22px', textAlign: 'right' }}>
                          <span style={{ color: TEXT3, fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>View →</span>
                        </td>
                      </tr>
                    )
                  })}
