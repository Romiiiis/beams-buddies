...
export default function RevenuePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!userData) return
      const { data } = await supabase.from('invoices').select('*, customers(first_name, last_name)').eq('business_id', userData.business_id).order('created_at', { ascending: false })
      setInvoices(data || [])
      setLoading(false)
    }
    load()
  ...
  const monthlyRevenue = last6.map(m => {
    const total = paid.filter(i => { const d = new Date(i.paid_at || i.created_at); return d.getMonth() === m.month && d.getFullYear() === m.year }).reduce((s, i) => s + i.total, 0)
    return { ...m, total }
  })
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflowY: 'auto' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: isMobile ? '28px' : '34px', fontWeight: '800', color: WHITE, letterSpacing: '-0.8px', lineHeight: 1 }}>Revenue</div>
          </div>
            ...
            <>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4, minmax(0,1fr))', gap: '12px' }}>
                  {[
                    { label: 'Total collected', value: `$${totalRevenue.toLocaleString('en-AU', { minimumFractionDigits: 0 })}`, sub: 'paid invoices', accent: '#10B981', valColor: '#064E3B' },
                    { label: 'Outstanding', value: `$${totalOutstanding.toLocaleString('en-AU', { minimumFractionDigits: 0 })}`, sub: 'awaiting payment', accent: '#3B82F6', valColor: '#1E3A8A' },
                    { label: 'Overdue', value: `$${totalOverdue.toLocaleString('en-AU', { minimumFractionDigits: 0 })}`, sub: totalOverdue > 0 ? 'needs follow-up' : 'all up to date', accent: totalOverdue > 0 ? '#EF4444' : TEAL, valColor: totalOverdue > 0 ? '#7F1D1D' : TEXT },
                    { label: 'Total invoiced', value: `$${totalInvoiced.toLocaleString('en-AU', { minimumFractionDigits: 0 })}`, sub: 'all invoices', accent: TEAL, valColor: TEXT },
                  ].map(s => (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: '14px' }}>
                  ...
                  <div style={card}>
                    <div style={{ padding: '16px 22px', borderBottom: `1px solid ${BORDER}` }}>
                      <div style={{ fontSize: '16px', fontWeight: '700', color: TEXT }}>Monthly revenue</div>
                      <div style={{ fontSize: '12px', color: TEXT3, marginTop: '2px' }}>Paid revenue over the last 6 months</div>
                    </div>
                    <div style={{ padding: '20px 22px' }}>
                      {monthlyRevenue.map(m => (
                        <div key={m.label} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                          <div style={{ fontSize: '12px', color: TEXT3, width: '72px', textAlign: 'right', flexShrink: 0, fontWeight: '500' }}>{m.label}</div>
                          <div style={{ flex: 1, height: '10px', background: '#F3F4F6', borderRadius: '5px', overflow: 'hidden' }}>
                            <div style={{ width: `${Math.round((m.total / maxMonthly) * 100)}%`, height: '100%', background: TEAL, borderRadius: '5px', transition: 'width 0.3s' }}/>
                          </div>
                          <div style={{ fontSize: '13px', fontWeight: '700', color: TEXT, width: '74px', textAlign: 'right', flexShrink: 0 }}>
                  ...
                  <div style={card}>
                    <div style={{ padding: '16px 22px', borderBottom: `1px solid ${BORDER}` }}>
                      <div style={{ fontSize: '16px', fontWeight: '700', color: TEXT }}>Top customers</div>
                    ...
                    <div style={{ padding: '20px 22px' }}>
                      {topCustomers.length === 0 ? (
                        ...
                        <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                          <div style={{ fontSize: '12px', color: TEXT3, width: '100px', flexShrink: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: '500' }}>{c.name}</div>
                          <div style={{ flex: 1, height: '10px', background: '#F3F4F6', borderRadius: '5px', overflow: 'hidden' }}>
                            <div style={{ width: `${Math.round((c.total / maxCustomer) * 100)}%`, height: '100%', background: TEAL, borderRadius: '5px', transition: 'width 0.3s' }}/>
                          </div>
                          <div style={{ fontSize: '13px', fontWeight: '700', color: TEXT, width: '74px', textAlign: 'right', flexShrink: 0 }}>
              ...
              <div>
                ...
                <div style={card}>
                  <div style={{ padding: '16px 22px', borderBottom: `1px solid ${BORDER}` }}>
                    <div style={{ fontSize: '16px', fontWeight: '700', color: TEXT }}>Invoice breakdown</div>
                  ...
                  <div style={{ padding: '4px 22px 10px' }}>
                    {[
                      ...
                      { label: 'Draft', value: invoices.filter(i => i.status === 'draft').length, amount: invoices.filter(i => i.status === 'draft').reduce((s: number, i: any) => s + i.total, 0), color: '#6B7280', bg: '#F3F4F6' },
                    ].map(row => (
                      <div key={row.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 0', borderBottom: `1px solid ${BORDER}`, gap: '12px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                          <span style={{ background: row.bg, color: row.color, padding: '3px 10px', borderRadius: '6px', fontSize: '12px', fontWeight: '700', flexShrink: 0 }}>{row.value}</span>
                          <span style={{ fontSize: '13px', color: TEXT2, fontWeight: '500' }}>{row.label}</span>
                        </div>
                        <span style={{ fontSize: '14px', fontWeight: '700', color: row.color, flexShrink: 0 }}>
