...
const TYPE = {
  ...
  section: { fontSize: '10px', fontWeight: 800, letterSpacing: '0.14em' as const, textTransform: 'uppercase' as const, color: TEXT3 },
  ...
  valueMd: { fontSize: '20px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em' as const, lineHeight: 1 },
...
function IconPhone({ size = 14 }: { size?: number }) {
  return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M22 16.9v3a2 2 0 0 1-2.2 2 19.8 19.8 0 0 1-8.6-3 19.4 19.4 0 0 1-6-6 19.8 19.8 0 0 1-3-8.7A2 2 0 0 1 4.2 2h3a2 2 0 0 1 2 1.7l.5 3a2 2 0 0 1-.6 1.8L7.8 9.8a16 16 0 0 0 6.4 6.4l1.3-1.3a2 2 0 0 1 1.8-.6l3 .5A2 2 0 0 1 22 16.9Z" stroke="currentColor" strokeWidth="1.9" strokeLinecap="round" strokeLinejoin="round"/></svg>
}
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ display: 'block', transform: 'rotate(-90deg)' }}>
        ...
        {arcs.map(arc => (
          <circle key={arc.label} cx={cx} cy={cy} r={r} fill="none" stroke={arc.color} strokeWidth={hovered === arc.label ? thickness + 6 : thickness} strokeDasharray={`${arc.sweep} ${circumference}`} strokeDashoffset={-arc.startAngle} strokeLinecap="butt" style={{ transition: 'stroke-width 0.18s ease, opacity 0.18s ease', opacity: hovered && hovered !== arc.label ? 0.35 : 1, cursor: 'pointer' }} onMouseEnter={() => setHovered(arc.label)} onMouseLeave={() => setHovered(null)} />
        ))}
      </svg>
...
function AppointmentsBarChart({ data, stats, dueSoonCount, isMobile }: { data: { label: string; total: number; completed: number }[]; stats: { jobsThisMonth: number; overdue: number; units: number }; dueSoonCount: number; isMobile: boolean }) {
  const [hovered, setHovered] = useState<string | null>(null)
  const chartMax = useMemo(() => Math.max(...data.map(d => Math.max(d.total, d.completed)), 1), [data])
  const CHART_H = 160
  const ticks = useMemo(() => { const top = Math.max(4, chartMax); return [top, Math.round(top * 0.5), 0] }, [chartMax])
  ...
  return (
    <>
      ...
      <div style={{ borderRadius: '14px', background: 'linear-gradient(180deg,#FFFFFF 0%,#FAFBFC 100%)', border: `1px solid ${BORDER}`, padding: '16px 16px 12px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '28px 1fr', gap: '8px', alignItems: 'stretch' }}>
          <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: CHART_H, paddingBottom: '2px' }}>
            {ticks.map((t, i) => <span key={i} style={{ fontSize: '10px', fontWeight: 700, color: TEXT3, lineHeight: 1, textAlign: 'right' }}>{t}</span>)}
          </div>
          <div style={{ position: 'relative' }}>
            {[0, 50, 100].map((pct, i) => <div key={i} style={{ position: 'absolute', left: 0, right: 0, top: `${pct}%`, borderTop: `1px ${i === 2 ? 'solid' : 'dashed'} #E8EDF3`, zIndex: 0 }} />)}
            <div style={{ position: 'relative', zIndex: 1, display: 'grid', gridTemplateColumns: 'repeat(12, minmax(0,1fr))', gap: '6px', height: CHART_H, alignItems: 'end' }}>
              {data.map(item => {
                ...
                return (
                  <div key={item.label} onMouseEnter={() => setHovered(item.label)} onMouseLeave={() => setHovered(null)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-end', gap: '4px', height: '100%', position: 'relative' }}>
                    {isActive && item.total > 0 && (
                      <div style={{ position: 'absolute', bottom: CHART_H - 4, left: '50%', transform: 'translateX(-50%)', background: '#0B1220', color: '#FFFFFF', fontSize: '10px', fontWeight: 800, padding: '4px 7px', borderRadius: '7px', whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 10, boxShadow: '0 4px 12px rgba(0,0,0,0.18)' }}>
                        {item.total} jobs · {item.completed} done
                      </div>
                    )}
                    <div style={{ width: '100%', height: totalH, position: 'relative', borderRadius: '6px 6px 3px 3px', background: isActive ? 'linear-gradient(180deg,#D4DCE6 0%,#C8D3DF 100%)' : 'linear-gradient(180deg,#EEF2F6 0%,#E2E8F0 100%)', transition: 'background 0.15s ease', overflow: 'hidden' }}>
                      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: compH, borderRadius: '6px 6px 3px 3px', background: isActive ? 'linear-gradient(180deg,#28C4B5 0%,#1F9E94 100%)' : 'linear-gradient(180deg,#2BBDB0 0%,#1F9E94 100%)', boxShadow: isActive ? '0 2px 10px rgba(31,158,148,0.30)' : '0 2px 6px rgba(31,158,148,0.15)', transition: 'background 0.15s ease' }} />
                    </div>
                    <div style={{ fontSize: '10px', fontWeight: isActive ? 800 : 700, color: isActive ? TEXT2 : TEXT3, transition: 'color 0.15s ease', marginTop: '4px' }}>{item.label}</div>
                  </div>
                )
              })}
            </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
      ...
      <div style={{ display: 'grid', gap: '6px' }}>
        {distribution.map(item => {
          ...
          return (
            <div key={item.label} onMouseEnter={() => setHovered(item.label)} onMouseLeave={() => setHovered(null)} style={{ display: 'grid', gridTemplateColumns: '1fr auto', alignItems: 'center', gap: '10px', padding: '9px 12px', borderRadius: '12px', background: isHov ? '#F4F8FA' : '#FCFCFD', border: `1px solid ${isHov ? '#D0DCE8' : BORDER}`, cursor: 'default', transition: 'background 0.15s ease, border-color 0.15s ease' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '9px', minWidth: 0 }}>
                <span style={{ width: 10, height: 10, borderRadius: '3px', background: item.color, flexShrink: 0, boxShadow: isHov ? `0 0 0 2px ${item.color}44` : 'none', transition: 'box-shadow 0.15s ease' }} />
                <span style={{ fontSize: '12px', fontWeight: 700, color: isHov ? TEXT : TEXT2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', transition: 'color 0.15s ease' }}>{item.label}</span>
              </div>
...
export default function DashboardPage() {
  ...
  const dueSoonCount = useMemo(() => allJobs.filter(j => { if (!j.next_service_date) return false; const days = getDays(j.next_service_date); return days >= 0 && days <= 30 }).length, [allJobs])
  ...
  const quickActionStyle: React.CSSProperties = { border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, borderRadius: '10px', height: '38px', padding: '0 14px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '8px', boxShadow: '0 1px 2px rgba(15,23,42,0.02)', whiteSpace: 'nowrap' as const }
  ...
  const monthlyAppointments = useMemo(() => {
    ...
    allJobs.forEach(job => {
      ...
      if (job.next_service_date) { const due = new Date(job.next_service_date); if (!Number.isNaN(due.getTime()) && due <= new Date()) base[due.getMonth()].completed += 1 }
    })
    return base
  }, [allJobs])
  ...
  const revenueDistribution = useMemo(() => {
    ...
    if (Object.values(buckets).every(v => v === 0)) { buckets.Service = 4; buckets.Installation = 3; buckets.Quote = 2; buckets.Repair = 2; buckets.Other = 1 }
  ...
  return (
    <div style={{ display: 'flex', height: '100vh', fontFamily: FONT, background: BG, overflow: 'hidden' }}>
      ...
      {/* ── KEY FIX: single scroll container, -webkit-overflow-scrolling for smooth mobile ── */}
      <div style={{ flex: 1, minWidth: 0, overflowY: 'auto', overflowX: 'hidden', background: BG, WebkitOverflowScrolling: 'touch' as any }}>
        <div style={{ display: 'flex', flexDirection: 'column', padding: isMobile ? '14px' : '16px', gap: '12px', paddingBottom: '40px' }}>
          ...
          <div style={{ ...shellCard, padding: isMobile ? '18px 16px 16px' : '22px 24px 20px', background: HEADER_BG, border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <div style={{ fontSize: isMobile ? '28px' : '34px', lineHeight: 1, letterSpacing: '-0.04em', fontWeight: 900, color: '#FFFFFF', marginBottom: '8px' }}>Dashboard</div>
            ...
            {/* ── MOBILE FIX: buttons always in a row, scroll horizontally if needed ── */}
            <div style={{ marginTop: '14px', display: 'flex', flexDirection: 'row', gap: '8px', overflowX: 'auto', paddingBottom: '2px', WebkitOverflowScrolling: 'touch' as any }}>
              <button onClick={() => router.push('/dashboard/jobs')} style={{ ...quickActionStyle, background: TEAL, color: '#FFFFFF', border: 'none', boxShadow: '0 6px 14px rgba(31,158,148,0.20)', flexShrink: 0 }}>
                <IconSpark size={16} /> Add job
              </button>
              <button onClick={() => router.push('/dashboard/quotes')} style={{ ...quickActionStyle, background: 'rgba(255,255,255,0.06)', color: '#FFFFFF', border: '1px solid rgba(255,255,255,0.10)', boxShadow: 'none', flexShrink: 0 }}>
                <IconInvoice size={16} /> New quote
              </button>
              <button onClick={() => router.push('/dashboard/schedule')} style={{ ...quickActionStyle, background: 'rgba(255,255,255,0.06)', color: '#FFFFFF', border: '1px solid rgba(255,255,255,0.10)', boxShadow: 'none', flexShrink: 0 }}>
          ...
          {/* Top stat cards */}
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(12, minmax(0, 1fr))', gap: '12px' }}>
            {topCards.map(item => (
              <div key={item.label} style={{ ...panelCard, gridColumn: isMobile ? 'span 1' : 'span 3', minHeight: isMobile ? 120 : 148, display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                  <div>
                    <div style={{ ...TYPE.label, marginBottom: '6px' }}>{item.tag}</div>
                    <div style={{ ...TYPE.title, fontSize: isMobile ? '12px' : '14px', fontWeight: 800, marginBottom: '8px' }}>{item.label}</div>
                  </div>
                  <div style={iconWrap(item.accent)}>{item.icon}</div>
                </div>
                <div>
                  <div style={{ ...TYPE.valueLg, fontSize: isMobile ? '24px' : '30px', color: item.accent }}>{item.value}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(12, minmax(0, 1fr))', gap: '12px', alignItems: 'start' }}>
            <div style={{ ...panelCard, gridColumn: isMobile ? 'span 1' : 'span 8' }}>
              <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', flexDirection: isMobile ? 'column' : 'row', gap: '6px', marginBottom: '14px' }}>
                <div>
          ...
          <div style={{ ...panelCard }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '10px', marginBottom: '12px' }}>
              ...
              <button onClick={() => router.push('/dashboard/schedule')} style={{ height: '34px', borderRadius: '10px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, fontSize: '12px', fontWeight: 700, padding: '0 12px', cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '8px', boxShadow: '0 1px 2px rgba(15,23,42,0.02)' }}>
                    ...
                    <div key={job.id} onClick={() => router.push(`/dashboard/customers/${job.customer_id}`)} style={{ borderRadius: '14px', padding: '14px', background: WHITE, border: `1px solid ${BORDER}`, cursor: 'pointer', display: 'grid', gap: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', minWidth: 0 }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                          ...
                          <div style={{ ...TYPE.bodySm, marginTop: '2px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{job.brand || 'Unit'} {job.capacity_kw ? `${job.capacity_kw}kW` : ''}</div>
                        </div>
                      </div>
                      <div style={{ display: 'grid', gap: '8px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                          <div style={{ ...TYPE.bodySm, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{job.customers?.suburb || 'No suburb'}</div>
          ...
          <div style={{ ...panelCard }}>
            <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', flexDirection: isMobile ? 'column' : 'row', gap: '10px', marginBottom: '12px' }}>
              ...
              <button onClick={() => router.push('/dashboard/customers')} style={{ height: '34px', borderRadius: '10px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, fontSize: '12px', fontWeight: 700, padding: '0 12px', cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '8px', boxShadow: '0 1px 2px rgba(15,23,42,0.02)' }}>
                View all <IconArrow size={15} />
              </button>
            </div>
            <div style={{ display: 'grid', gap: '10px' }}>
              {recent.length === 0 ? (
                ...
                recent.slice(0, 5).map((job, i) => {
                  ...
                  return (
                    <div key={job.id} onClick={() => router.push(`/dashboard/customers/${job.customer_id}`)} style={{ borderRadius: '14px', padding: isMobile ? '14px' : '14px 16px', background: WHITE, border: `1px solid ${BORDER}`, cursor: 'pointer', display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1.2fr) minmax(0,1fr) minmax(0,0.9fr) auto', gap: '12px', alignItems: 'center' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', minWidth: 0 }}>
                        ...
                        <div style={{ minWidth: 0 }}>
                          <div style={{ ...TYPE.titleSm, fontSize: '13px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{job.customers?.first_name} {job.customers?.last_name}</div>
                      ...
                      <div style={{ justifySelf: isMobile ? 'start' : 'end', display: 'inline-flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ background: status.bg, color: status.color, padding: '7px 10px', borderRadius: '999px', fontSize: '11px', fontWeight: 800, whiteSpace: 'nowrap', display: 'inline-block', letterSpacing: '0.02em' }}>{status.label}</span>
                      </div>
                    </div>
                  )
                })
