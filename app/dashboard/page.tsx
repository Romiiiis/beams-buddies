...
function VisitCalendarWidget({ jobs, isMobile, onDateClick }: {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    cells.push({
      day,
      dateKey: toYMD(new Date(viewYear, viewMonth, day)),
    })
  }
  ...
  const nextBooking = Object.entries(jobsByDate)
    .filter(([dateKey]) => dateKey >= todayKey)
    .sort(([a], [b]) => a.localeCompare(b))
    .flatMap(([dateKey, dayJobs]) => dayJobs.map(job => ({ dateKey, job })))
    [0]

  const compactUpcoming = monthJobEntries
    .flatMap(([dateKey, dayJobs]) => dayJobs.slice(0, 1).map(job => ({ dateKey, job, dayJobs })))
    .slice(0, 4)

  return (
    <div style={{ background: WHITE }}>
      <div style={{ padding: isMobile ? '14px 14px 12px' : '16px 18px 14px', borderBottom: `1px solid ${BORDER}`, background: WHITE }}>
        <div style={{ display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', gap: '12px' }}>
          <div style={{ minWidth: 0 }}>
            <div style={{ fontSize: '10px', fontWeight: 900, color: TEAL_DARK, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '6px' }}>
              Booking calendar
            </div>

            <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', flexWrap: 'wrap' }}>
              <div style={{ fontSize: isMobile ? '20px' : '22px', fontWeight: 900, color: TEXT, letterSpacing: '-0.045em', lineHeight: 1 }}>
                {MONTH_NAMES_FULL[viewMonth]}
              </div>
              <div style={{ fontSize: '13px', fontWeight: 700, color: TEXT3 }}>
                {viewYear}
              </div>
            </div>

            <div style={{ fontSize: '11px', fontWeight: 600, color: TEXT3, marginTop: '7px', lineHeight: 1.35 }}>
              {monthBookingCount} booking{monthBookingCount !== 1 ? 's' : ''} this month
              {nextBooking ? (
                <>
                  <span style={{ opacity: 0.45 }}> · </span>
                  Next: {parseDateLocal(nextBooking.dateKey)?.toLocaleDateString('en-AU', { day: 'numeric', month: 'short' })}
                </>
              ) : null}
            </div>
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
            <button onClick={goToday} style={{ height: 32, padding: '0 11px', borderRadius: '9px', border: `1px solid ${BORDER}`, background: '#F8FAFC', color: TEXT2, fontSize: '11px', fontWeight: 800, cursor: 'pointer', fontFamily: FONT }}>
              Today
            </button>

            <button onClick={prevMonth} style={{ width: 32, height: 32, borderRadius: '9px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <IconChevronLeft size={13} />
            </button>

            <button onClick={nextMonth} style={{ width: 32, height: 32, borderRadius: '9px', border: `1px solid ${BORDER}`, background: WHITE, color: TEXT2, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginTop: '14px' }}>
          {[
            ...
            { label: 'This month', value: monthBookingCount },
            { label: 'Booked days', value: monthJobEntries.length },
          ].map(item => (
            <div key={item.label} style={{ border: `1px solid ${BORDER}`, background: '#F8FAFC', borderRadius: '12px', padding: isMobile ? '9px 8px' : '10px 11px' }}>
              <div style={{ fontSize: isMobile ? '17px' : '19px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em', lineHeight: 1 }}>
                {item.value}
              </div>
              <div style={{ fontSize: '10px', fontWeight: 700, color: TEXT3, marginTop: '5px', whiteSpace: 'nowrap' }}>
                {item.label}
              </div>
            </div>
          ))}
      ...
      <div style={{ padding: isMobile ? '12px 12px 14px' : '14px 16px 16px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '6px', marginBottom: '7px' }}>
          {DAY_NAMES_SHORT.map(day => (
            <div key={day} style={{ textAlign: 'center', fontSize: '10px', fontWeight: 800, color: TEXT3, letterSpacing: '0.03em' }}>
              {day}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '6px' }}>
          {cells.map((cell, i) => {
            if (!cell.day || !cell.dateKey) {
              return <div key={i} style={{ height: isMobile ? 42 : 46 }} />
            }
            ...
            return (
              <button
                ...
                onClick={() => {
                  if (hasJobs || isToday) {
                    onDateClick(new Date(viewYear, viewMonth, cell.day!), jobsOnDay)
                  }
                }}
                style={{
                  height: isMobile ? 42 : 46,
                  borderRadius: '12px',
                  border: isToday ? `1px solid ${TEAL}` : hasJobs ? `1px solid rgba(31,158,148,0.35)` : `1px solid ${BORDER}`,
                  background: isToday ? TEAL : hasJobs ? '#F2FBFA' : WHITE,
                  color: isToday ? WHITE : hasJobs ? TEAL_DARK : TEXT2,
                  ...
                  gap: '4px',
                  position: 'relative',
                  opacity: !isToday && !hasJobs && isPast ? 0.45 : 1,
              ...
              >
                <span style={{ fontSize: '12px', fontWeight: isToday || hasJobs ? 900 : 700, lineHeight: 1 }}>
                  {cell.day}
                </span>

                {hasJobs ? (
                  <span style={{ minWidth: 18, height: 15, padding: '0 5px', borderRadius: '999px', background: isToday ? 'rgba(255,255,255,0.18)' : WHITE, border: isToday ? '1px solid rgba(255,255,255,0.22)' : `1px solid rgba(31,158,148,0.25)`, color: isToday ? WHITE : TEAL_DARK, fontSize: '8px', fontWeight: 900, lineHeight: '13px' }}>
                    {count}
                  </span>
                ) : (
      ...
      <div style={{ borderTop: `1px solid ${BORDER}`, background: '#FCFCFD', padding: isMobile ? '12px' : '14px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
          <div>
            <div style={{ fontSize: '13px', fontWeight: 900, color: TEXT, letterSpacing: '-0.02em' }}>
              Month agenda
            </div>
            <div style={{ fontSize: '10px', fontWeight: 600, color: TEXT3, marginTop: '2px' }}>
              Tap a booking to view the day
            </div>
          </div>

          <span style={{ padding: '4px 8px', borderRadius: '999px', background: TEAL_LIGHT, color: TEAL_DARK, fontSize: '10px', fontWeight: 900 }}>
            {compactUpcoming.length} shown
          </span>
        </div>

        {compactUpcoming.length === 0 ? (
          <div style={{ border: `1px dashed ${BORDER}`, borderRadius: '13px', background: WHITE, padding: '18px', textAlign: 'center' }}>
            <div style={{ width: 34, height: 34, borderRadius: '11px', margin: '0 auto 9px', background: '#F8FAFC', display: 'flex', alignItems: 'center', justifyContent: 'center', color: TEXT3 }}>
              <IconCalendar size={15} />
            </div>
            <div style={{ fontSize: '12px', fontWeight: 800, color: TEXT2 }}>
              No bookings this month
            </div>
            <div style={{ fontSize: '10px', fontWeight: 600, color: TEXT3, marginTop: '3px' }}>
              New scheduled jobs will show here.
            </div>
          </div>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {compactUpcoming.map(({ dateKey, job, dayJobs }) => {
              const d = parseDateLocal(dateKey)
              const name = `${job.customers?.first_name || ''} ${job.customers?.last_name || ''}`.trim() || 'Customer'
              const jobLabel = (job.job_type || job.equipment_type || 'Service').replace(/_/g, ' ')
              const dateDay = d ? d.getDate() : ''
              const dateMonth = d ? d.toLocaleDateString('en-AU', { month: 'short' }) : ''
              const dateWeekday = d ? d.toLocaleDateString('en-AU', { weekday: 'short' }) : ''
              ...
              return (
                ...
                >
                  <div style={{ width: 46, height: 46, borderRadius: '13px', background: TEAL_LIGHT, color: TEAL_DARK, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <div style={{ fontSize: '9px', fontWeight: 900, lineHeight: 1 }}>
                      {dateMonth}
                    </div>
                  ...
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: '12px', fontWeight: 900, color: TEXT, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      {name}
                    </div>
                    <div style={{ fontSize: '10px', fontWeight: 600, color: TEXT3, marginTop: '3px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', textTransform: 'capitalize' }}>
                      {dateWeekday} · {jobLabel}
                      {job.customers?.suburb ? ` · ${job.customers.suburb}` : ''}
                    </div>
                  </div>

                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: TEAL_DARK, fontSize: '10px', fontWeight: 900, whiteSpace: 'nowrap' }}>
                    View
