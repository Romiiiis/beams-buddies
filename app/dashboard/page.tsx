...
const TYPE = {
  label:  { fontSize: '10px', fontWeight: 800, letterSpacing: '0.09em' as const, textTransform: 'uppercase' as const, color: TEXT3 },
  bodySm: { fontSize: '12px', fontWeight: 500, color: TEXT3, lineHeight: 1.5 },
}
...
export default function DashboardPage() {
  ...
  function statusPill(d: string | null) {
    ...
    return { label: 'Scheduled', bg: '#F1F5F9', color: '#475569' }
  }
    ...
    <div style={{ display: 'flex', fontFamily: FONT, background: BG, minHeight: '100vh' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, background: BG }}>
        <div style={{
          ...
          {/* ── HEADER ──────────────────────────────────────────────────── */}
          <div style={{ ...card, padding: isMobile ? '20px 18px 18px' : '24px 28px 22px', background: HEADER_BG, border: '1px solid rgba(255,255,255,0.08)' }}>
            <div style={{ fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,0.55)', marginBottom: '7px', letterSpacing: '0.02em' }}>{todayStr}</div>
            <div style={{ fontSize: isMobile ? '26px' : '34px', lineHeight: 1, letterSpacing: '-0.04em', fontWeight: 900, color: WHITE, marginBottom: '9px' }}>Dashboard</div>
            <div style={{ fontSize: '13px', fontWeight: 500, lineHeight: 1.55, color: 'rgba(255,255,255,0.65)', maxWidth: '760px' }}>
              Track customers, service due dates, invoices, and jobs from one control centre.
            </div>
            <div style={{ marginTop: '16px', display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
              <button onClick={() => router.push('/dashboard/jobs')} style={{ height: '36px', padding: '0 16px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '7px', background: TEAL, color: WHITE, border: 'none', borderRadius: '10px' }}>
                <IconSpark size={14}/> Add job
              </button>
              <button onClick={() => router.push('/dashboard/quotes')} style={{ height: '36px', padding: '0 16px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '7px', background: 'rgba(255,255,255,0.07)', color: WHITE, border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px' }}>
                <IconInvoice size={14}/> New quote
              </button>
              <button onClick={() => router.push('/dashboard/schedule')} style={{ height: '36px', padding: '0 16px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, display: 'inline-flex', alignItems: 'center', gap: '7px', background: 'rgba(255,255,255,0.07)', color: WHITE, border: '1px solid rgba(255,255,255,0.12)', borderRadius: '10px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(4, 1fr)', gap: '14px' }}>
            ...
            <div style={cardP}>
              ...
              <div style={{ fontSize: '22px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em', marginBottom: '18px' }}>
                <span>{stats.jobsThisMonth}</span>
                <span style={{ fontSize: '13px', fontWeight: 600, color: TEXT3 }}> scheduled</span>
              </div>
            ...
            <div style={cardP}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '6px' }}>
                <div style={{ ...TYPE.label }}>Revenue</div>
                <button onClick={() => router.push('/dashboard/revenue')} style={cardArrowBtn()}>
                  <IconExternalLink size={14}/>
                </button>
              </div>
              <div style={{ marginBottom: '6px' }}>
                <span style={{ fontSize: '28px', fontWeight: 900, color: TEXT, letterSpacing: '-0.05em' }}>
                  ${invoiceStats.collected > 0 ? (invoiceStats.collected / 1000).toFixed(1) + 'k' : '0'}
                </span>
                <span style={{ fontSize: '12px', fontWeight: 600, color: TEXT3, marginLeft: 7 }}>this month</span>
              </div>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', marginBottom: '12px', padding: '4px 8px', borderRadius: '7px', background: '#E6F7F6' }}>
                <span style={{ color: TEAL }}><IconTrendUp size={11}/></span>
                <span style={{ fontSize: '11px', fontWeight: 800, color: TEAL_DARK }}>
                  ${invoiceStats.outstanding > 0 ? (invoiceStats.outstanding / 1000).toFixed(1) + 'k' : '0'} outstanding
                </span>
              </div>
              <Sparkline data={sparkData} color={TEAL} width={160} height={50}/>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginTop: '12px' }}>
                {revenueBreakdown.slice(0, 3).map(rb => (
                  <div key={rb.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: rb.color, flexShrink: 0 }}/>
                    <span style={{ fontSize: '12px', fontWeight: 600, color: TEXT2, flex: 1 }}>{rb.label}</span>
                    <span style={{ fontSize: '12px', fontWeight: 700, color: TEXT3 }}>${(rb.value / 1000).toFixed(1)}k</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={cardP}>
              ...
              <div style={{ marginBottom: '6px' }}>
                ...
                <span style={{ fontSize: '12px', fontWeight: 600, color: TEXT3, marginLeft: 7 }}>this week</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '16px' }}>
                {revenueBreakdown.map(rb => (
                  <div key={rb.label} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: rb.color, flexShrink: 0 }}/>
                    <span style={{ fontSize: '12px', fontWeight: 600, color: TEXT2, flex: 1 }}>{rb.label}</span>
                    <span style={{ fontSize: '12px', fontWeight: 700, color: TEXT }}>${(rb.value / 1000).toFixed(1)}k</span>
                  </div>
                ))}
              </div>
              <div style={{ marginTop: '16px', paddingTop: '14px', borderTop: `1px solid ${BORDER}`, fontSize: '12px', fontWeight: 600, color: TEXT3 }}>
            ...
            <div style={cardP}>
              ...
              <div style={{ fontSize: '22px', fontWeight: 900, color: TEXT, letterSpacing: '-0.04em', marginBottom: '18px' }}>
                {stats.overdue > 0
                  ? <><span style={{ color: '#B91C1C' }}>{stats.overdue}</span> Overdue</>
                  : <span style={{ color: TEAL }}>All Clear</span>
                }
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '18px' }}>
                <div onClick={() => router.push('/dashboard/jobs')}
                  style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 13px', borderRadius: '11px', background: '#FEF2F2', border: '1px solid #FECACA', cursor: 'pointer' }}>
                ...
                <div onClick={() => router.push('/dashboard/invoices')}
                  ...
                  <div style={{ flex: 1, fontSize: '12px', fontWeight: 700, color: '#7F1D1D' }}>
                    {invoiceStats.outstanding > 0
                      ? <><span style={{ fontWeight: 900 }}>${invoiceStats.outstanding.toLocaleString('en-AU', { minimumFractionDigits: 0 })}</span> overdue</>
                      : 'No overdue invoices'}
                  </div>
                  <IconChevronRight size={13}/>
                </div>
              ...
              <button onClick={() => router.push('/dashboard/jobs')}
                style={{ width: '100%', height: '36px', background: '#F8FAFC', border: `1px solid ${BORDER}`, borderRadius: '10px', fontSize: '12px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, color: TEXT2 }}>
                View all
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 296px', gap: '16px', alignItems: 'start' }}>
            ...
            <div style={card}>
              <div style={{ padding: '18px 20px 14px', borderBottom: `1px solid ${BORDER}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
                ...
                <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap' }}>
                  {[
                    ...
                    <div key={item.label} style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                      ...
                      <span style={{ fontSize: '13px', fontWeight: 900, color: TEXT }}>{item.val}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              {upcoming.length === 0 ? (
                <div style={{ padding: '36px 20px', textAlign: 'center', color: TEXT3, fontSize: '13px' }}>No upcoming appointments.</div>
                ...
                return (
                  <div key={job.id} onClick={() => router.push(`/dashboard/customers/${job.customer_id}`)}
                    style={{ display: 'grid', gridTemplateColumns: isMobile ? '64px 1fr auto' : '76px 1fr auto auto', gap: '12px', alignItems: 'center', padding: '15px 20px', borderBottom: `1px solid ${BORDER}`, cursor: 'pointer', transition: 'background 0.12s' }}
                    ...
                    <div style={{ minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '7px', marginBottom: '3px', flexWrap: 'wrap' }}>
                        ...
                        <span style={{ fontSize: '12px', color: TEXT3 }}>{job.customers?.suburb || ''}</span>
                      </div>
                      <div style={{ fontSize: '11px', fontWeight: 500, color: TEXT3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {job.next_service_date ? new Date(job.next_service_date).toLocaleDateString('en-AU', { day: 'numeric', month: 'short' }) : 'No date'} · {brand}
                      </div>
                    </div>
                    {!isMobile && <div style={{ fontSize: '12px', fontWeight: 500, color: TEXT3, whiteSpace: 'nowrap' }}>{job.job_type || 'Service'}</div>}
                    {isFirst ? (
                      <button onClick={e => { e.stopPropagation(); router.push(`/dashboard/customers/${job.customer_id}`) }}
                        style={{ height: '32px', padding: '0 11px', background: '#F8FAFC', border: `1px solid ${BORDER}`, borderRadius: '8px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, color: TEXT2, display: 'inline-flex', alignItems: 'center', gap: '4px', whiteSpace: 'nowrap' }}>
                  ...
                  <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap' }}>
                    {([['next7', 'Next 7 Days'], ['due', 'Jobs Due'], ['attime', 'At time']] as const).map(([key, lbl]) => (
                      <button key={key} onClick={() => setActiveTab(key)}
                        style={{ height: '30px', padding: '0 10px', borderRadius: '8px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, border: `1px solid ${activeTab === key ? TEAL : BORDER}`, background: activeTab === key ? '#E6F7F6' : WHITE, color: activeTab === key ? TEAL_DARK : TEXT3 }}>
                        {lbl}
                      </button>
                    ))}
                  </div>
                  <button onClick={() => router.push('/dashboard/jobs')}
                    style={{ height: '30px', padding: '0 10px', background: '#F8FAFC', border: `1px solid ${BORDER}`, borderRadius: '8px', fontSize: '11px', fontWeight: 700, cursor: 'pointer', fontFamily: FONT, color: TEXT2, display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
            ...
            {/* REVENUE STATS */}
            <div style={cardP}>
              <div style={{ fontSize: '15px', fontWeight: 800, color: TEXT, marginBottom: '18px' }}>Revenue Stats</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {revenueBreakdown.map(rb => (
                  <div key={rb.label} onClick={() => router.push('/dashboard/revenue')}
                    style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '11px 0', borderBottom: `1px solid ${BORDER}`, cursor: 'pointer' }}>
              ...
              <div style={{ marginTop: '14px', padding: '14px', borderRadius: '12px', background: '#E6F7F6', border: '1px solid #C4E8E5' }}>
                ...
                <div style={{ ...TYPE.bodySm, marginTop: '3px' }}>{invoiceStats.paidCount} paid invoices</div>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 340px', gap: '16px', alignItems: 'start' }}>
            ...
            <div style={card}>
              ...
              {recent.length === 0 ? (
                <div style={{ padding: '32px 20px', textAlign: 'center', color: TEXT3, fontSize: '13px' }}>No customers yet.</div>
              ) : recent.map((job, i) => {
                const name = `${job.customers?.first_name || ''} ${job.customers?.last_name || ''}`.trim() || 'Customer'
                const initials = (job.customers?.first_name?.[0] || '') + (job.customers?.last_name?.[0] || '')
                const sp = statusPill(job.next_service_date)
                // Alternating avatar accent palettes — teal family
                const avBg  = ['#E6F7F6', '#EEF2F6', '#EAF0FB', '#F0EEF9', '#E9F4EE'][i % 5]
                const avColor = ['#0A5C58', '#334155', '#1D4B91', '#4A3889', '#0D5C36'][i % 5]
                return (
                  <div key={job.id} onClick={() => router.push(`/dashboard/customers/${job.customer_id}`)}
                    style={{
                      display: 'grid',
                      gridTemplateColumns: isMobile ? 'auto 1fr auto' : 'auto 1fr 120px 120px auto',
                      gap: '14px',
                      alignItems: 'center',
                      padding: '16px 20px',
                    ...
                    {/* Name + location */}
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: '14px', fontWeight: 700, color: TEXT, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '3px' }}>
                        {name}
                      </div>
                      <div style={{ fontSize: '12px', color: TEXT3, display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                        {job.customers?.suburb && <span>{job.customers.suburb}</span>}
                        {job.customers?.phone && !isMobile && (
                          <>
                            <span style={{ opacity: 0.4 }}>·</span>
                            <span style={{ display: 'flex', alignItems: 'center', gap: '3px' }}>
                              <IconPhone size={11}/>{job.customers.phone}
                            </span>
                          </>
                        )}
                      </div>
                    ...
                    {!isMobile && (
                      <div style={{ minWidth: 0 }}>
                        <div style={{ fontSize: '10px', fontWeight: 700, color: TEXT3, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '3px' }}>Unit</div>
                        <div style={{ fontSize: '13px', fontWeight: 600, color: TEXT2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                          {job.brand || 'Unit'}{job.capacity_kw ? ` ${job.capacity_kw}kW` : ''}
                        </div>
                      </div>
                    )}

                    {/* Next service — desktop only */}
                    {!isMobile && (
                      <div>
                        <div style={{ fontSize: '10px', fontWeight: 700, color: TEXT3, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '3px' }}>Next service</div>
                        <div style={{ fontSize: '13px', fontWeight: 600, color: TEXT2 }}>
            ...
            <div style={card}>
              ...
              <div style={{ padding: '16px 20px', borderBottom: `1px solid ${BORDER}` }}>
                <div style={{ fontSize: '26px', fontWeight: 900, color: invoiceStats.outstanding > 0 ? '#991B1B' : TEXT, letterSpacing: '-0.04em', lineHeight: 1 }}>
                  ${invoiceStats.outstanding > 0 ? invoiceStats.outstanding.toLocaleString('en-AU', { minimumFractionDigits: 0 }) : '0'}
                </div>
                <div style={{ fontSize: '12px', fontWeight: 600, color: TEXT3, marginTop: '4px' }}>
                  outstanding · {invoiceStats.overdueCount} overdue
                </div>
              </div>

              {invoiceStats.allInvoices.length === 0 ? (
                <div style={{ padding: '28px 20px', textAlign: 'center', color: TEXT3, fontSize: '13px', fontWeight: 600 }}>
                ...
                return (
                  <div key={inv.id || i} onClick={() => router.push('/dashboard/invoices')}
                    style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '15px 20px', borderBottom: `1px solid ${BORDER}`, cursor: 'pointer', transition: 'background 0.12s' }}
                    onMouseEnter={e => (e.currentTarget.style.background = '#F8FAFC')}
                    onMouseLeave={e => (e.currentTarget.style.background = WHITE)}>
                    ...
                    <div style={{
                      ...
                      border: `1px solid ${isOverdue ? '#FECACA' : BORDER}`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      color: isOverdue ? '#B91C1C' : TEXT3,
                    }}>
                      <IconInvoice size={15}/>
                    </div>

                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '13px', fontWeight: 700, color: TEXT, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginBottom: '3px' }}>
                        {name}
                      </div>
                    ...
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                      <div style={{ fontSize: '14px', fontWeight: 800, color: isOverdue ? '#991B1B' : TEXT, marginBottom: '4px' }}>
                        ${amt.toLocaleString('en-AU', { minimumFractionDigits: 0 })}
                      </div>
                      <span style={{
